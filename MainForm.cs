...
namespace SantronWinApp
{
    ...
    public partial class MainForm : Form
    {
        ...
        private bool _useBleEmg = true; // Set to true to use Bluetooth EMG instead of DAQ
        //======= EMG ============
        ...
        private void BuildScaleMaxOverlays()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (cb == null)
                {
                    ...
                    // ✅ set initial min/max scale
                    double minScale = (_useBleEmg && lane.Equals("Emg", StringComparison.OrdinalIgnoreCase))
                                        ? -currentMax
                                        : 0;

                    _liveChart.SetChannelScaleByName(lane, minScale, currentMax);

                    // ---- fill scale options ----
                    if (_scaleOptions.TryGetValue(lane, out double[] opts) && opts?.Length > 0)
                    {
                    ...
                    cb.SelectedIndexChanged += OnScaleMaxChanged;
            ...
            foreach (string laneKey in toRemove)
            {
                if (_scaleCombos.TryGetValue(laneKey, out ComboBox gone))
                {
                    gone.Parent?.Controls.Remove(gone);
                    gone.Dispose();
        ...
        private void OnScaleMaxChanged(object sender, EventArgs e)
        {
            ...
            // Determine scale
            double minScale = (_useBleEmg && laneName.Equals("Emg", StringComparison.OrdinalIgnoreCase))
                                ? -newMax
                                : 0;

            _liveChart.SetChannelScaleByName(laneName, minScale, newMax);

            // Pressure channels group
            ...
            {
                if (lane.Equals(laneName, StringComparison.OrdinalIgnoreCase))
                    continue;
                ...
                // Pressure channels always 0 → max
    ...
    if (isBleFile)
    {
        ...
        private int _emgDebugCounter = 0;
        private int _emgDebugCounter2 = 0;
        private double _emgDebugMin = double.MaxValue;
        private double _emgDebugMax = double.MinValue;
        private const double EMG_BASELINE_CLAMP_UV = 100.0; // values below this = treated as silence
        private void StartEmg()
        {
            if (_emgStreaming) return;
            StopEmg();
            ...
            // ── DEBUG: BLE connection state ───────────────────────────────────────
            _emgSrc.OnConnectionStateChanged += state =>
            {
                Debug.WriteLine($"[BLE] Connection state: {state}");
            };
            ...
            _emgPointHandler = processedValue =>
            {
                // ── Guard: BLE must be connected ─────────────────────────────────
                if (_useBleEmg && (_emgSrc == null || !_emgSrc.IsConnected)) return;

                double filtered;

                if (_useBleEmg)
                {
                    // BLE: already HP + BP + Notch processed by EmgLiteEngine
                    double notched = _emgNotchFilter.Process(processedValue);
                    filtered = _emgHighPassFilter.Process(notched);
                    // Envelope: rectify (absolute value) then low-pass smooth
                    // This is what Chords NPG-lite plots — flat at rest, rises on contraction
                    double rectified = Math.Abs(filtered);
                    _emgEnvelope = EMG_ENVELOPE_ALPHA * rectified + (1.0 - EMG_ENVELOPE_ALPHA) * _emgEnvelope;

                    _filteredEmgValue = _emgEnvelope;  // plot envelope, not raw filtered
                    _lastEmgUv = _emgEnvelope;
                }
                else
                {
                    // DAQ
                ...
                // ── DEBUG: log every 500 samples (~1/sec) ────────────────────────

                _emgDebugCounter2++;
                if (_emgDebugCounter2 >= 2500)
                {
                    _emgDebugCounter2 = 0;
                    Debug.WriteLine(
                        $"[EMG-RANGE] min={_emgDebugMin:F1}  max={_emgDebugMax:F1}  " +
                        $"mid={((_emgDebugMin + _emgDebugMax) / 2.0):F1}  " +
                        $"mode={(_useBleEmg ? "BLE" : "DAQ")}");
                    _emgDebugMin = double.MaxValue;
                    _emgDebugMax = double.MinValue;
                }
                ...
                // ── Chart plotting ────────────────────────────────────────────────
                //  if (_liveChart == null || _isPlaybackMode || _isPaused) return;
                if (_liveChart == null || _isPlaybackMode || _isPaused || !_isRecording) return;
                var channelNames = _liveChart.GetChannelNames();
                int emgLaneIdx = -1;
                for (int i = 0; i < channelNames.Count; i++)
                {
                ...
                if (emgLaneIdx < 0)
                {
                    Debug.WriteLine("[EMG] EMG lane NOT FOUND in chart channels.");
                    return;
                }
            ...
            if (_useBleEmg)
            {
                // BLE mode: engine wraps _emgSrc, wire handler then start
                _emgEngine.OnEmgPoint += _emgPointHandler;
                _emgEngine.Start();
                Debug.WriteLine($"[EMG-START] BLE mode — engine started, connected={_emgSrc?.IsConnected}");
            }
            else
            {
                // DAQ mode: engine already set up externally, just wire handler
                _emgEngine.OnEmgPoint += _emgPointHandler;
                _emgEngine.Start();
                Debug.WriteLine($"[EMG-START] DAQ mode — engine started");
            }
        ...
        private void btnChannel0_Click(object sender, EventArgs e)
        {
            ...
            // Also zero channel 6 if available
            if (_lastRawCounts.Length > 6)
            {
                _orch.Proc.ZeroChannel(6, _lastRawCounts[6]);
            }
        }
        ...
        private void btnPumpSpUp_MouseUp(object sender, MouseEventArgs e)
        {
        ...
        private void btnPumpSpDown_MouseUp(object sender, MouseEventArgs e)
        {
            //Start Code For Add Delay
            if ((DateTime.Now - _lastClickTime).TotalMilliseconds < 500)
            {
        ...
        //Pump Start MouseUP Button
        private void btnPumpStart3_MouseUp(object sender, MouseEventArgs e)
        {
            PumpStart();
        }
        ...
        //Arm Button MouseUP
        private async void btnUppSpeed_MouseUp(object sender, MouseEventArgs e)
        {
        ...
        private void DownloadPdf_Click(object sender, EventArgs e)
        {
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                try
                {
                    // Create a NEW PrintDocument instance for PDF generation
                    // This is crucial - don't reuse the existing _doc
                    PrintDocument pdfDoc = new PrintDocument();

                    // Copy the necessary settings from the original document
                    pdfDoc.DefaultPageSettings = _doc.DefaultPageSettings;
                    pdfDoc.OriginAtMargins = _doc.OriginAtMargins;

                    // Re-attach the print page handler
                    pdfDoc.PrintPage += (s, es) =>
                    {
                        // Reset page counter for this new print job
                        int currentPdfPage = 1;

                        // You need to access the printer's internal state
                        // The printer should have its own page counter
                        _printer.PrintPage(s, es, currentPdfPage);
                    ...
                    // Reset the printer's internal state before printing
                    _currentPage = 1; // Reset your page counter
